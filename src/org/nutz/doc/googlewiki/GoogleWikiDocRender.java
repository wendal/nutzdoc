...
//		}
	...
	private static String ele2String(ZEle ele){
		...
		if(ele.hasStyle() && ele.getStyle().hasFont()){
			...
			if(font.isBold()) text = GoogleWikiBuilder.wrapFont_Bold(text);
			if(font.isItalic()) text = GoogleWikiBuilder.wrapFont_Italic(text);
			if(font.isSup()) text = GoogleWikiBuilder.wrapFont_SuperScript(text);
			if(font.isSub()) text = GoogleWikiBuilder.wrapFont_SubScript(text);
			if(font.isStrike()) text = GoogleWikiBuilder.wrapFont_Strike(text);
		}
		if(ele.hasHref()){
